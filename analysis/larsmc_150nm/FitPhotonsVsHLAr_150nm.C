...
void GraphAttLensVsHLArMin(double attenuation_length_0to3mm, std::vector<double> attenuation_lengths_15to18mm, std::vector<double> attenuation_lengths_30to33mm, std::vector<double> attenuation_lengths_42to45mm, std::vector<double> attenuation_lengths_45to48mm, std::vector<double> attenuation_lengths_48to51mm)
{
    ...
    g1->GetYaxis()->SetRangeUser(160.0, 10000.0);
    g1->Draw("AP");
    ...
    TLine *l1 = new TLine(0, 164.531, 105, 164.531);
    ...
    legend->Draw();
    cmain->Update();
    cmain->Print("/nashome/a/abreaux/Downloads/larsmc_150nm/AttenuationLength_Vs_HLArMin_150nm.root");
}
...
void FitPhotonsVsHLAr_150nm()
{
    ...
    for (int i = 0; i < 19; i++)
    {
        std::string ginput_name = "/nashome/a/abreaux/Downloads/larsmc_150nm/gPhotonCount_vs_r2_" + r_range_names[i] + "_150nm.root";
        ...
        {  
            attenuation_lengths_30to33mm = Fit_Other(ginput_name);
        } 
        else if (r_range_names[i] == "42to45mm") 
        {  
            attenuation_lengths_42to45mm = Fit_Other(ginput_name);
        } 
        else if (r_range_names[i] == "45to48mm")
        {
            attenuation_lengths_45to48mm = Fit_Other(ginput_name);
        }
        else if (r_range_names[i] == "48to51mm")
        {
            attenuation_lengths_48to51mm = Fit_Other(ginput_name);
        }
        else {;}
    }
