...
void DoFit(std::string file_path)
{
    ...
    canvas->Update();
    canvas->Print("/nashome/a/abreaux/Downloads/larsmc_all/FitScatLens_Vs_AttLens.root");
...
{
    std::string file_path = "/nashome/a/abreaux/Downloads/larsmc_all/ScatLens_Vs_AttLens.root";
    DoFit(file_path);
}