...
void GraphAttLensVsHLArMin(double attenuation_length_0to3mm, std::vector<double> attenuation_lengths_15to18mm, std::vector<double> attenuation_lengths_30to33mm, std::vector<double> attenuation_lengths_42to45mm, std::vector<double> attenuation_lengths_45to48mm, std::vector<double> attenuation_lengths_48to51mm)
{
    ...
    g1->GetYaxis()->SetRangeUser(120.0, 1370.0);
    g1->Draw("AP");
    ...
    TLine *l1 = new TLine(0, 124.852, 105, 124.852);
    ...
    legend->Draw();
    cmain->Update();
    cmain->Print("/nashome/a/abreaux/Downloads/larsmc_140nm/AttenuationLength_Vs_HLArMin_140nm.root");
}
...
void FitPhotonsVsHLAr_140nm()
{
    ...
    for (int i = 0; i < 19; i++)
    {
        std::string ginput_name = "/nashome/a/abreaux/Downloads/larsmc_140nm/gPhotonCount_vs_r2_" + r_range_names[i] + "_140nm.root";
