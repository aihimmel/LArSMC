...
void GraphCorrection(const int n, std::vector<std::string> r_range_names, std::vector<double> corrected_attlens)
{
    ...
    for (int i = 0; i < r_range_names.size(); i++)
    {
        ...
        legend->Draw();
        std::string outfile_path = "/nashome/a/abreaux/Downloads/CorrectionAttLens_Vs_ScatLens_" + r_range_names[i] + ".root";
...
{
    std::string file_path = "/nashome/a/abreaux/Downloads/FitAttLens_Vs_ScatLens.root";
